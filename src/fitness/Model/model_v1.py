...
class Model:
    def __init__(self, current=5, cellDiamater=18, separation=1, initFlow=100, \
                 initTemperature=20, col_fluido=2, col_celda=1, n_fluido=2, n_celda=1,\
                 nmax=10):
        """
		"""
        ...
        self.z = 5 * 1e-3       # Corte de estudio    [m]
    ...
    def start(self):
        ...
        self.sp['diamTimesZ'] = self.cellDiameter_m * self.z
        self.sp['superficialArea'] = np.pi * self.sp['diamTimesZ']                 # Area de la celda
        self.sp['sPlusOne'] = self.separation + 1
        self.sp['controlVolArea'] = self.sp['sPlusOne'] * self.sp['diamTimesZ']    # Area volumen control eje z

        self.sp['vol'] = self.largo * self.sp['cellArea']                          # Volumen celda [m3]
        self.sp['volumetricQdot'] = (self.current**2) * self.R / self.sp['vol']    # Flujo de Calor volumetrico
        self.sp['qdot'] = self.sp['volumetricQdot'] * self.z * self.sp['cellArea'] # Flujo de Calor total corte
        self.sp['height'] = self.sp['doubleE'] + self.cellDiameter_m * \
                            (self.n_fluido + self.separation*self.n_celda)         # Altura del pack
        self.sp['entranceArea'] = self.sp['height'] * self.z                       # Area de entrada pack
        self.sp['innerArg'] = self.initFlow_m3_sg * self.z / \
                              (self.largo * self.sp['entranceArea'])
        ...
        self.vars_vec['rem'] = 0.0 * np.ones(self.col_fluido)                      # [adimensional]
        ...
        self.sp['dfMultiplicationTerm'] = self.sp['diamTimesZ'] * self.sp['initVelocity'] * \
                                               self.vars_vec['df'][0]
    ...
    def evolve(self):
        for _ in range(self.nmax):
            ...
            cdrag = self.individual['computeDragCoefficient'](self.sp['a'][0], self.vars_vec['rem'][0], \
                                                              self.sp['normalizedArea'], normalizedDF, self.col_fluido) #OJO Es el numero de columnas o la columna actual?
            ...
            frictionFactor =self.individual['computeFrictionFactor'](actualRem, self.separation, \
                                                                     actualVMF / self.sp['initVelocity'], normalizedDF)
            ...
            for i in range(self.col_fluido-1):
                ...
                normalizedDF = actualDF / 1.205
                frictionFactor = self.individual['computeFrictionFactor'](actualRem, self.separation, \
                                                                          self.vars_vec['vmf'][i] / self.sp['initVelocity'], normalizedDF)
                ...
                cdrag = self.individual['computeDragCoefficient'](self.sp['a'][0], actualRem, \
                                                                  self.sp['normalizedArea'], normalizedDF, self.col_fluido)
                actualVFSquared = (actualVF ** 2)
                nextFF = 0.5 * self.sp['diamTimesZ'] * actualDF * actualVFSquared * cdrag
                ...
                nu = self.individual['computeNusseltNumber']((i + 1) * 2*1.0, actualRem, self.sp['a'][2]) # OJO, ¿Cuál columna?
                iniFluidK = q_conductividad(actualTF)
                self.vars_vec['fluidK'][i] = iniFluidK
                h = nu * iniFluidK / self.cellDiameter_m # [W m^-2 K^-1]
                # Transferencia de energia99
        ...
        return np.array([self.vars_vec['vf'][1:], self.vars_vec['pf'][:-1], self.vars_vec['tc']])
    ...
    def load_individual(self, cdrag_tree, ffriction_tree, nnusselt_tree):
        '''
        :param cdrag_tree:     inputs(5) (a1, reynolds, normalizedArea, normalizedDensity, fluidColumn) 
        :param ffriction_tree: inputs(4) (reynolds, separation, normalizedVelocity, normalizedDensity)
        :param nnusselt_tree:  inputs(3) (Column, reynolds, a3)
        '''
