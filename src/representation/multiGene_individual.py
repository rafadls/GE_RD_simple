...
from algorithm.mapper import mapper_multiGene
from algorithm.parameters import params


class MultiIndividual(object):
    """
    A GE individual.
    """
    ...
    def __init__(self, genome, ind_tree, map_ind=True):
        ...
        if map_ind:
            ...
            self.phenotype, self.genome, self.tree, self.nodes, self.invalid, \
                self.depth, self.used_codons = mapper_multiGene(genome, ind_tree)
            self.phenotype_original = str(self.phenotype)
        else:
        ...
        self.mse = params['FITNESS_FUNCTION'].default_mse
        self.rmsePerColumn = "aun no"
    ...
    def deep_copy(self):
        ...
        new_ind = MultiIndividual(self.genome.copy(), new_tree, map_ind=False)
    ...
    def evaluate(self, optimization=False):
        ...
        evaluationResult = params['FITNESS_FUNCTION'](self, optimization=optimization)
        self.fitness = evaluationResult[0]
        self.mse = evaluationResult[1] 
